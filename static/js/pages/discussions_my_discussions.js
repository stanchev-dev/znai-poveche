(function () {
  ...
  const pageRoot = document.querySelector('.my-discussions-page');
  const listWrap = pageRoot?.querySelector('.vstack');
  const publishBtnHref = pageRoot?.querySelector('.discussions-cta-btn')?.getAttribute('href') || '/discussions/publish/';
  ...
  function renderEmptyState() {
    ...
    pageRoot.insertAdjacentHTML('beforeend', `
      <div class="card border-0 shadow-sm rounded-4 p-4 p-md-5 my-listings-empty-state">
        <h2 class="h4 mb-2">Нямаш публикувани дискусии.</h2>
        <p class="text-muted mb-3">Започни нова дискусия и попитай общността.</p>
        <div><a class="btn btn-brand-purple" href="${publishBtnHref}">+ Публикувай</a></div>
      </div>`;
  }
  ...
  document.addEventListener('click', async (event) => {
    ...
    if (!window.confirm('Сигурен ли си, че искаш да изтриеш тази дискусия?')) return;
